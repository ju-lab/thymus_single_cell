...
par(mfrow = c(1, 2))
tmp <- count_ercc.pass %>% mutate(ERCC_ID = names_ercc) %>%
  left_join(spike_in_concentration) %>%
  select(ERCC_ID, concentration, everything()) %>%
  gather(-ERCC_ID, -concentration, 
         key = "Cell", value = "raw_ERCC_count") %>%
  left_join(data.frame(Cell = colnames(count_ercc.pass), size_factor = st))
tmp %>% with(plot(log10(concentration), log10(raw_ERCC_count),
                  col = "#00000030"))

tmp <- count_ercc.norm %>% mutate(ERCC_ID = names_ercc) %>%
  left_join(spike_in_concentration) %>%
  select(ERCC_ID, concentration, everything()) %>%
  gather(-ERCC_ID, -concentration, 
         key = "Cell", value = "normalized_ERCC_count") %>%
  left_join(data.frame(Cell = colnames(count_ercc.norm), size_factor = st))
tmp %>% with(plot(log10(concentration), log10(normalized_ERCC_count),
                  col = "#00000030"))
tmp2 <- data.frame(
  cvsquared = apply(count_ercc.norm, 1, 
                    function(x) (sd(x, na.rm = T) / mean(x, na.rm = T))^2),
  avexp = apply(count_ercc.norm, 1, function(x) mean(x, na.rm = T))
)
plot(log10(tmp2$avexp), log10(tmp2$cvsquared))
tmp3 <- data.frame(
  cvsquared = apply(count_gene.norm, 1, 
                    function(x) (sd(x, na.rm = T) / mean(x, na.rm = T))^2),
  avexp = apply(count_gene.norm, 1, function(x) mean(x, na.rm = T))
)
plot(log10(tmp3$avexp), log10(tmp3$cvsquared), col = "#00000030")
